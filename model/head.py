...
class ClassificationHead(Mlp):
    ...
    def __init__(self, output_format="logits", **kwargs):
...
class BackgroundClassHead(ClassificationHead):
    ...
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        assert kwargs["out_dim"] == 2, (
            "out_dim must be 2 for background class head"
        )
...
class BoxSizeHead(Mlp):
    ...
    def __init__(self, **kwargs):
        ...
        assert kwargs["output_activation"] != "none", (
            "output_activation cannot be none for BoxSizeHead (needs to non-negative values)."
        )
...
class LocalizationHead(Mlp):
    ...
    def __init__(self, **kwargs):
        ...
        assert "out_dim" in kwargs, (
            "out_dim must be specified for LocalizationHead"
        )
        assert kwargs["out_dim"] == 3, "out_dim must be 3 for LocalizationHead"
...
class OrientationHead(Mlp):
    ...
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        assert "out_dim" in kwargs, (
            "out_dim must be specified for OrientationHead"
        )
        assert kwargs["out_dim"] == 1, "out_dim must be 1 for OrientationHead"
        self.clip_output = kwargs.get("clip_output", [-1.0, 1.0])
